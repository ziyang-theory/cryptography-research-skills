...
- When connecting generation to consumption, attach validity and authentication state to the count. Separate raw, candidate, checked, usable, discarded, and consumed outputs; include the work required to reach the claimed state.
...
Read [relation and rate method](references/relation-and-rate-method.md) for grouped outputs, usable-output accounting, cross-construction comparisons, or rate conversions. It contains optional worksheets and equations; use only the fields the question needs.
